...
// Reads the readable signal cookie set by /api/login alongside the httpOnly auth cookie.
// The httpOnly cookie does the actual security (middleware checks it server-side).
// This cookie just tells the UI "don't show the modal — this person is already in."
function isUnlocked(): boolean {
  if (typeof window === "undefined") return false;
  return document.cookie
    .split(";")
    .some((c) => c.trim().startsWith("portfolio-auth-ui=1"));
}
...
export default function ProtectedButton({
  ...
  function handleClick() {
    // Cookie already set (from ProtectedButton, /login page, or a return visit)? Skip modal.
    if (isUnlocked()) {
      router.push(href);
      return;
    }
  ...
  async function handleUnlock() {
    ...
    if (res.ok) {
      // The API sets both cookies. isUnlocked() will return true on the next click.
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                >
                  This section is password protected as it may include
                  confidential context. Some details are redacted, but the work
                  and outcomes are intact.
                </p>
                <p
                  className="type-body"
                  style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                {error && (
                  <p className="type-body" style={{ color: "#ef4444" }}>
                    That password is not correct. Try again or request a new
                    password.
                  </p>
                )}
              </div>
  ...
  return (
    <>
      <button
        ...
        style={{
          color: "rgb(var(--chip-active-text))",
          textDecoration: "none",
          border: "none",
        }}
      >
